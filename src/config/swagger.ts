...
const options: swaggerJsdoc.Options = {
  definition: {
    ...
    tags: [
      ...
      {
        name: "Chat",
        description: "In-app messaging. Chat is realtime over socket.io: connect to the API origin with the access\ntoken, as `auth.token` or an `Authorization: Bearer ...` header. This host rejects WebSocket upgrades, so connect with the polling transport (transports: ['polling'], upgrade: false); socket.io then runs over HTTP long-polling. See CHAT_REALTIME.md in the repo for a full client guide.\n\n**Server to client:** `new_message` ({ ...message, conversationId }), `conversation_updated`\n({ type: 'conversation', conversationId, lastMessage, unreadCount } or { type: 'notification', notification }), `messages_delivered`\n({ conversationId, messageIds, deliveredAt, recipientId }), `messages_read`\n({ conversationId, messageIds, readAt, readerId }), `message_deleted`\n({ conversationId, messageId, deletedBy }), `presence_update`\n({ userId, isOnline, lastSeenAt }, sent on connect and on disconnect), `user_typing` / `user_stop_typing`\n({ userId, conversationId }).\n\n**Client to server:** `join_conversation` (conversationId, optional ack gives { ok } or\n{ ok: false, error }), `leave_conversation` (conversationId), `typing` / `stop_typing`\n({ conversationId }).\n\nA socket joins every conversation its user belongs to on connect, so messages arrive without\ncalling `join_conversation`; joining only matters for typing indicators. Every event names the conversation `conversationId`; a message also keeps its own\n`conversation` field and the two match. Messages carry `deliveredAt` once the\nrecipient is connected and `readAt` once they open the conversation. Deleting a message\nemits `message_deleted` and then `conversation_updated`, so the list preview and the\nunread badge stay correct without a resync.",
      },
